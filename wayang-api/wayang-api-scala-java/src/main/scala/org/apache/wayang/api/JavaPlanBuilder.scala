...
import org.apache.wayang.basic.operators.{TableSource, ParquetSource, TextFileSource, KafkaTopicSource}
...
class JavaPlanBuilder(wayangCtx: WayangContext, jobName: String) {
  ...
  /**
    * Reads a parquet table and provides them as a dataset of [[Record]]s.
    *
    * @param source from that the [[Record]]s should be read
    * @return [[DataQuantaBuilder]] for the [[Record]]s in the parquet file.
    */
