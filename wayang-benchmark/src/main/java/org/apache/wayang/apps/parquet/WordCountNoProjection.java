...
public class WordCountNoProjection {
    ...
    public static void main(String[] args) throws IOException, URISyntaxException {
        try {
            ...
            var plan = planBuilder
                    .readParquet(new JavaParquetSource(args[0]))
                    .withName("Read parquet")
                    .flatMap(row -> Arrays.asList((row.getField(1).toString()).split("\\W+")))
            ...
            if (args.length > 1) {
              try (FileWriter fileWriter = new FileWriter(args[1], true)) {
                fileWriter.write(String.format("No Projection %s, %d\n", args[0], duration));
